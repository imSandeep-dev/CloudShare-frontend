                    ...
                    <div className="flex gap-3 w-full justify-center">
                        {file.isPublic && (
                            <button 
                                ...
                                title='Share Link' className="p-2 bg-white/90 rounded-full hover:bg-white transition-colors cursor-pointer text-purple-500 hover:text-purple-600">
                                <Copy size={18}/>
                            </button>
                        )}
                        {file.isPublic && (
                            <a title="View File" href={file.fileLocation} target="_blank" rel="noreferrer" className="p-2 cursor-pointer bg-white/90 rounded-full hover-bg-white transition-colors text-gray-700 hover:text-gray-900">
                                <Eye size={18}/>
                            </a>
                        )}
                        <button 
                            onClick={()=>onDownload(file)}
                            title="Download" className="p-2 bg-white/90 rounded-full cursor-pointer hover:bg-white transition-colors text-green-600 hover:text-green-700">
                            <Download size={18}/>
                        </button>
                        <button
